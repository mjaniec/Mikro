...
gpBool gpGestureDetector_onTouchEvent(gpGestureDetector* gestureDetector, gpMotionEvent* ev)
{
		...
		gpInt action = ev->actionType;
        gpFloat y = gpMotionEvent_getY(ev, 0);
        gpFloat x = gpMotionEvent_getX(ev, 0);

        if(gestureDetector->mVelocityTracker == null)
        {
            gestureDetector->mVelocityTracker = gpVelocityTracker_createInstance();
        }

        gpVelocityTracker_addMovement(gestureDetector->mVelocityTracker, ev);

        gpBool handled = false;
        switch (action & gpMotionEvent_ACTION_MASK)
        {
			case gpMotionEvent_ACTION_POINTER_DOWN:
				break;
			case gpMotionEvent_ACTION_POINTER_UP:
				break;
        }
        return handled;
}
