...
gpBool _gpCheckFingerForZoom(gpVector*vec,gpPoint *target,gpPoint*azimut,gpBool dir_in,gpInt bounce){
	...
	for(gpInt i=bounce+1; i<size; ++i){
		...
		if(
				gpMath_Abs(gpSub(angle,baseAngle))>gpMath_PI4 &&
				gpMath_Abs(gpSub(gpSub(angle,baseAngle),gpMath_2PI))>gpMath_PI4	//near 0 angle
		)return false;
		if(dir_in && currentDist>previousDist)return false;
		if(!dir_in&& currentDist<previousDist)return false;
...
gpBool gpTryZoom (gpMotionEvent*event,gpRecognizeContext*context){
	...
	gpPrintVectorOfPoints(context->finger1,"one")$r0;
	gpPrintVectorOfPoints(context->finger2,"two")$r0;
